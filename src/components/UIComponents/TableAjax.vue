  ...
  <div class="row m-0">
      ...
      >
        ...
        >
          <template slot-scope="props">
            <span
              v-if="
                column.json_field && typeof props.row[column.prop] === 'object'
              "
            >
              <p
                v-for="key in Object.keys(props.row[column.prop])"
                :key="key"
                class="mb-5px"
              >
                {{ key.toUpperCase() }}:
                <span v-if="column.type === 'number'">
                  <router-link
                    ...
                    >{{
                      $util.formatNumber(props.row[column.prop][key])
                    }}</router-link
                  >
                  <span v-else>{{
                    $util.formatNumber(props.row[column.prop][key])
                ...
                <span v-else-if="column.type === 'select'">
                  <badge
                    v-if="
                      $util.parseType(column.prop, props.row[column.prop][key])
                    "
                    :type="
                      $util.parseType(column.prop, props.row[column.prop][key])
                    "
                  >
                    {{ parseStatus(column.prop, props.row[column.prop][key]) }}
                  </badge>
                  <span v-else>
                    {{ parseStatus(column.prop, props.row[column.prop][key]) }}
                  </span>
                ...
                <span v-else-if="column.type === 'badge'">
                  <badge :type="props.row['type'] || 'default'">{{
                    props.row[column.prop][key]
                  }}</badge>
                </span>
                <span v-else-if="column.type === 'html'">
                  <span v-html="props.row[column.prop][key]"></span>
                </span>
                <span v-else-if="column.type === 'link'">
                  <a :href="props.row[column.prop][key]" target="_blank">{{
                    props.row[column.prop][key]
                  }}</a>
                </span>
                <span v-else-if="column.type === 'money'">
                  <span v-if="props.row[column.prop][key]">{{
                    $util.formatVndShort(props.row[column.prop][key])
                  }}</span>
                </span>
                <span v-else-if="column.type === 'date'">
                  <span>{{
                    props.row[column.prop][key] &&
                    $util.formatDate(props.row[column.prop][key])
                  }}</span>
                </span>
                <span v-else-if="column.type === 'datetime'">
                  <span>{{
                    props.row[column.prop][key] &&
                    $util.formatDateTime(props.row[column.prop][key])
                  }}</span>
                </span>
                <span v-else>
                  <router-link
                  ...
                  <span v-else>{{ props.row[column.prop][key] }}</span>
                </span>
              </p>
            </span>
            <span v-else-if="column.type === 'image'" align="center">
            ...
            <span v-else-if="column.type === 'link'">
              <a :href="props.row[column.prop]" target="_blank">{{
                props.row[column.prop]
              }}</a>
            </span>
            <span v-else-if="column.type === 'money'">
    ...
    <div class="col-sm-4 pagination-info">
      <p class="text-entries text-center">
        {{ $t("el.table.paginationText", [from, to, pagination.total]) }}
      </p>
    </div>
    <div class="col-sm-4">
      <p-pagination
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      this.loading = true;
      try {
        this.pagination.total = await this.remoteMethod({
    ...
    sortChange(column, prop, order) {
      this.orders = [
        { rule: "prop", op: order == "descending" ? "desc" : "asc" },
      ];
      this.fetchData();
    },
