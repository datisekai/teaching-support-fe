  ...
  export default {
    ...
    methods: {
      remoteMethod(query) {
        ...
        this.loading = true;
        api.get(buildQuery).then(resp => {
          let data = resp.data;
          if (!data.error) {
            this.dataSource = this.buildDataSource([], this.dataField ? data.data[this.dataField] : data.data);
          }
          this.loading = false;
        }).catch(e => {
          this.loading = false;
      ...
      buildDataSource(selectedList, newList) {
        ...
        return list.map((obj,index) => ({
          label: (this.displayField && obj[this.displayField]) || obj[this.field] || obj.name || obj.title,
