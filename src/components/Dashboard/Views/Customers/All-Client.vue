...
import clientSchemas from './client-schemas';
...
export default {
  ...
  computed: {
    clients() {
      const rows = this.$store.state.clients;
      return dtHelper.filterByRules(rows, this.filterOutput);
    },
  },
  data() {
    const initFiledArrays = ['id', 'logo', 'name', 'updated_at', 'status'];
    const columnDefs = dtHelper.buildInitFields(clientSchemas, initFiledArrays);
    ...
    return {
      ...
      filter: {},
      rules: dtHelper.buildRules(clientSchemas),
    };
  },
  mounted() {
    this.$store.dispatch('fetchClient');

    this.$store.dispatch('setPageTitle', 'khách hàng tiêu biểu');
  ...
  methods: {
    ...
    updateStatus(rows, status) {
      const self = this;
      this.$util.updateStatusAll('client', rows, status).then((result) => {
    ...
    exportExcel(){
      let self = this;
      this.$util.exportExcel('client', self.$store.state.clients);
    },
  },
};
</script>
