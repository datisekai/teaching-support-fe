...
export default {
  ...
  data() {
    ...
    return {
      filterOutput: [],
      columnDefs: dtHelper.buildInitFields(
        notificationSchemas,
        initFiledArrays
      ),
      actions: [
  ...
  methods: {
    ...
    async loadNotifications() {
      return this.$store.dispatch("fetchNotifications", {
        types: "campaign,flashsale,news"
      });
    },
    edit(index, row) {
    ...
    async send(index, row) {
      MessageBox.confirm(
        "Thông báo sẽ được gửi cho tất cả người sử dụng trong hệ thống.\nBạn có chắc chắn muốn gửi không?",
        "Warning",
        {
        ...
        try {
          const sendingResult = await this.$store.dispatch(
            "sendNotification",
            row.id
          );
          Notification({
    ...
    async batchAction(rows, action) {
      try {
        await Promise.all(
          rows.map(r => this.$store.dispatch(`${action}Notification`, r.id))
        );
        await this.loadNotifications();
        Notification({
          title: "Success",
