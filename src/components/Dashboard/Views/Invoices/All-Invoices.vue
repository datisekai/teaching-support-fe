...
import invoiceForm from './invoice-schemas';
...
export default {
  ...
  computed: {
    invoices() {
      const rows = this.$store.state.invoices.map(c => {
        c.customer_name = c.customer && c.customer.profile && c.customer.profile.fullname;
        c.notebook = c.receiptForm.notebook;
        c.receiptNo = c.receiptForm && c.receiptForm.receiptNo || '-';
        c.returnedAt = c.receiptForm && c.receiptForm.returnedAt || '-';
        c.invoiceFormStatus = c.status;
        if (c.total === 0) {
          c.formattedMoney = 'Miễn phí';
        }
        else {
          c.formattedMoney = this.$util.formatMoney(c.total) + ' đ';
        }
        return c;
      });
  ...
  data() {
    const initFiledArrays = ['id', 'receiptNo', 'notebook', 'customer_name', 'returnedAt', 'formattedMoney', 'invoiceFormStatus'];
    const columnDefs = dtHelper.buildInitFields(invoiceForm, initFiledArrays);
    const tableActions = [];
    ...
    return {
      ...
      filter: {},
      rules: dtHelper.buildRules(invoiceForm),
      actionsTable: [],
      dialog: {
  ...
  mounted() {
    this.$store.dispatch('fetchInvoices', { perpage: 1000 });
    this.$store.dispatch('setPageTitle', 'invoice');
  ...
  methods: {
    ...
    remove(index, row) {
      MessageBox.confirm('Bạn có chắc chắn xóa không?', 'Warning', {
        ...
        this.$store.dispatch('removeInvoice', row.id).then((res) => {
          Notification ({
            title: 'Success',
