<template>
  <div class="row">
    ...
    >
      <el-form
        :model="formCreateWarrantyCode"
        data-vv-scope="formCreateWarrantyCode"
      >
        <el-form-item :label="$t('el.formCard.model')">
          <my-select
            ...
            :class="
              errors.has('formCreateWarrantyCode.modelId')
                ? 'border-danger'
                : ''
            "
          ></my-select>
          <span
            class="text-danger"
            v-if="errors.has('formCreateWarrantyCode.modelId')"
            >{{ errors.first("formCreateWarrantyCode.modelId") }}</span
          >
        </el-form-item>
        <el-form-item :label="$t('el.formCard.amount')">
          <el-input
            ...
            :class="
              errors.has('formCreateWarrantyCode.amount') ? 'border-danger' : ''
            "
          >
          </el-input>
          <span
            class="text-danger"
            v-if="errors.has('formCreateWarrantyCode.amount')"
            >{{ errors.first("formCreateWarrantyCode.expiredIn") }}</span
          >
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button
          ...
          @click="storeWarrantyCode"
          :disabled="!formCreateWarrantyCode.modelId"
    ...
    >
      ...
      >
        <el-form-item :label="$t('el.formCard.code')">
          <el-input
            ...
            :class="
              errors.has('formValidateWarrantyCode.code') ? 'border-danger' : ''
            "
          ></el-input>
          <span
            class="text-danger"
            v-if="errors.has('formValidateWarrantyCode.code')"
            >{{ errors.first("formValidateWarrantyCode.code") }}</span
          >
        </el-form-item>
        <div v-if="formValidateWarrantyCode.result">
          ...
          <el-form-item>
            <div v-if="formValidateWarrantyCode.result.error">
              <p>
                <strong>{{ $t("el.formCard.error") }}: </strong>
            ...
            <div v-else>
              ...
              <p>
                ...
                {{
                  formValidateWarrantyCode.result.data.isActivated
                    ? "✔ Đã kích hoạt"
                    : "✗ Chưa kích hoạt"
                }}
              </p>
              <div v-if="formValidateWarrantyCode.result.data.isActivated">
                <p>
                  ...
                  {{ formValidateWarrantyCode.result.data.customer.fullname }} -
                  {{ formValidateWarrantyCode.result.data.customer.phone }}
                </p>
                <p>
                  <strong>{{ $t("el.formCard.activatedDate") }}: </strong>
                  {{
                    $util.formatDateTime(
                      formValidateWarrantyCode.result.data.activatedDate
                    )
                  }}
                </p>
                <p>
                  ...
                  {{
                    $util.formatDateTime(
                      formValidateWarrantyCode.result.data.expiredDate
                    )
                  }}
                </p>
              </div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialog.formValidateVisible = false">{{
          $t("el.button.cancel")
        }}</el-button>
        <el-button
          type="warning"
          ...
          :disabled="!formValidateWarrantyCode.code"
          >{{ $t("el.button.validate") }}</el-button
        >
      </span>
    </el-dialog>
  </div>
    ...
    storeWarrantyCode() {
      this.$validator.validateAll("formCreateWarrantyCode").then(result => {
        if (result) {
          ...
          this.formCreateWarrantyCode.amount = parseInt(
            this.formCreateWarrantyCode.amount
          );
          this.$store
            .dispatch("storeWarrantyCode", this.formCreateWarrantyCode)
            .then(
              res => {
    ...
    exportPath: {
      ...
      handler(newVal, oldVal) {
        if (newVal) {
          ...
          if (
            this.$util.checkPermissionAction(permissionSchemas, "exportExcel")
          ) {
            currentAction.push({
              label: "exportExcel",
              type: "warning",
