      ...
      >
        ...
        <!-- <el-form-item :label="$t('el.formCard.usageLimitQty')">
          <el-input
            type="text"
            class="full-width"
            data-vv-name="formUpdateProduct.usageLimitQty"
...
export default {
  ...
  async mounted() {
    ...
    this.$store.dispatch("fetchFlashsaleDetail", id);
    this.$store.dispatch("setPageTitle", "flashsaleUpdate");
  ...
  computed: {
    ...mapState(["flashsaleDetail"]),
    canAddProduct() {
      return (
        this.formProduct.productId &&
        this.formProduct.salePrice &&
        this.formProduct.usageLimitQty
      );
    }
  },
  methods: {
    cancel() {
      this.$router.push({ name: "AllFlashsales" });
    },
    addProduct() {
    ...
    remove() {
      ...
      MessageBox.confirm("Bạn có chắc chắn xóa không?", "Warning", {
        ...
        .then(() => {
          self.$store.dispatch("removeFlashsale", self.form.id).then(res => {
            Notification({
              title: "Success",
            ...
            self.$router.push({ name: "AllFlashsales" });
          });
        })
        .catch(() => {
          Notification({
    ...
    async save() {
      const formFlashSaleValidate = await this.$validator.validateAll("form");
      if (formFlashSaleValidate) {
    ...
    resetFormProduct() {
      ...
      this.formProduct.usageLimitQty = 999999;
    },
    ...
    mapProductToForm(product) {
      const minMaxPrice = product.variants.reduce(
        (minMax, v) => {
      ...
      const rangePrice =
        minMaxPrice[0] !== minMaxPrice[1]
          ? minMaxPrice
              .map(price => this.$util.formatMoney(price) + " đ")
              .join(" - ")
  ...
  watch: {
    ...
    flashsaleDetail(newVal) {
      if (newVal) {
        this.form = {
          ...this.form,
          ...newVal,
          startEnd: [new Date(newVal.startDate), new Date(newVal.expiredDate)]
        };
        this.products = newVal.products.map(r => {
          r.image = r.images.length && r.images[0].medium;
          const variants = r.variants || [];
          const minMaxPrice = variants.reduce(
            (minMax, v) => {
              minMax[0] = Math.min(minMax[0], v.regularPrice);
              minMax[1] = Math.max(minMax[1], v.regularPrice);
              return minMax;
          ...
          r.rangePrice =
            minMaxPrice[0] !== minMaxPrice[1]
              ? minMaxPrice
                  ...
                  .join(" - ")
              : this.$util.formatMoney(minMaxPrice[0]) + " đ";
          r.createdAt = this.$util.formatDateTime(r.createdAt);
