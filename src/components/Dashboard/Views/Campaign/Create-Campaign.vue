    ...
    <form class="offset-xl-2 col-xl-8 col-md-12" data-vv-scope="form">
      ...
      <div class="card">
        ...
        <div class="card-body form-card">
          ...
          <div v-if="form.discountType != 'free_shipping'"> 
            ...
            <div class="row">
              <div class="col-sm-12 p-0">
                <label class="required">{{$t('el.formCard.applyTo')}}</label>
                <my-select
                  class="full-width"
                  ...
                  :class="errors.has('form.type_relation')?'border-danger':''"
            ...
            <div class="row">
              <div v-if="form.typeRelation == 'category'" class="col-sm-12 p-0">
                ...
                <select-ajax
                  ...
                  :attribute="{type: 'categories', field: 'name', key: 'id', valueKey: 'id', multiple: true}"
                >
                </select-ajax>
                <span class="text-danger" v-if="errors.has('form.categoryIds')">{{ errors.first('categoryIds') }}</span>
              </div>
              <div v-else class="col-sm-12 p-0">
                <label class="required">{{$t('el.formCard.applyFor')}}</label>
                <select-ajax
                  key="products"
                  ...
                  :attribute="{type: 'products', field: 'name', key: 'id', valueKey: 'id', multiple: true}"
                >
                </select-ajax>
      ...
      <div class="card">
        ...
        <div class="card-body form-card">
          ...
          <!-- <div class="row">
            <el-switch v-model="isCreateNotification" :active-value='true' :inactive-value='false' :active-text="$t('el.formCard.createNotification')">
            </el-switch>
          </div> -->
        </div>
      </div>
    </form>
    <!-- <form v-if="isCreateNotification" class="offset-xl-2 col-xl-8 col-md-12" data-vv-scope="noti">
      <form-card
        :group="notiScheduleFormGroup"
        v-model="notiScheduleForm"
        >
...
import {
  Select, Option, Button, Input, DatePicker, Radio, Notification, RadioGroup, Switch, Col
} from 'element-ui';
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('setPageTitle', 'campaignCreate');
  ...
  methods: {
    ...
    async save() {
      ...
      if (result) {
        ...
        try {
          const campaign = await this.$store.dispatch('storeCampaign', data);
          Notification ({
            title: 'Success',
