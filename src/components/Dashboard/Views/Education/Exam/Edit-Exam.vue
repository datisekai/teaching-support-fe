    ...
    <div class="col-md-6 col-12">
      <div class="card">
        ...
        >
          ...
          <el-button type="primary" size="small" @click="showCreateQuestion()"
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("fetchExamDetail", id);
  ...
  methods: {
    ...
    async createQuestion() {
      ...
      this.questionForm.examId = this.form.id;
      this.questionForm.answers = this.questionForm.answers.map((i, index) => {
        i.label = this.getLabel(index);
        return i;
      });
      try {
        let q = await this.$store.dispatch("createQuestion", this.questionForm);
        this.form.questions.push(
          Object.assign({}, this.questionForm, { id: q.id })
        );
    ...
    async updateQuestion() {
      ...
      for (let i = 0; i < this.form.questions.length; i++) {
        if (i === this.selectedQuestionIndex) {
          ...
          this.$store.dispatch("updateQuestion", this.form.questions[i]);
        }
      }
    ...
    removeQuestion(index, question) {
      MessageBox.confirm("Bạn có chắc chắn xóa câu hỏi không?", "Warning", {
        ...
        .then(() => {
          ...
          this.$store.dispatch("removeQuestion", question.id).then(
            res => {
              Message({
                message: "Đã xóa",
                type: "success"
