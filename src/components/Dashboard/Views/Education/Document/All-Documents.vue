...
export default {
  ...
  data() {
    const initFiledArrays = ['id','title','description', 'status', 'createdAt'];
    const columnDefs = dtHelper.buildInitFields(documentSchemas, initFiledArrays);
  ...
  methods: {
    ...
    remove(index, row) {
      MessageBox.confirm('Bạn có chắc chắn xóa không?', 'Warning', {
        ...
        this.$store.dispatch('removeDocument', row.id).then((res) => {
          Message({
              message: 'Đã xóa',
              type: 'success'
            });
    ...
    updateStatus(rows, status) {
      const self = this;
      this.$util.updateStatusAll('document', rows, status).then((result) => {
