<template lang="html">
  <div class="card">
    ...
    <div class="card-body form-card">
      ...
      <div class="row">
        <div class="col-sm-12 p-0">
          <el-checkbox :disabled="disabledUpdate" v-model="hasCoupon"
            >Coupon</el-checkbox
          >
          <select-ajax
        ...
        <div class="col-sm-12 p-0">
          ...
          <div class="col-sm-12 p-0">
                ...
                <div>
                  <div>
                    <strong>Câu {{ index + 1 }} </strong> ({{
                      $t(`el.questionTypes.${question.type}`)
                    }}): {{ question.content }}
                  </div>
                  <div
            ...
            >
              ...
              <div v-if="formAddQuestion.type == 'text'"></div>
              <div v-else>
            ...
            >
              ...
              <div v-if="formUpdateQuestion.type == 'text'"></div>
              <div v-else>
...
export default {
  ...
  methods: {
    ...
    async createQuestion() {
      try {
        if (
          this.formAddQuestion.type === "radio" ||
          this.formAddQuestion.type === "checkbox"
        ) {
          this.formAddQuestion.answerLines = this.answerLines.filter(
            line => line.content != ""
          );
        }
        let q = await this.$store.dispatch(
        ...
        this.form.questions.push(
          Object.assign({}, this.formAddQuestion, { id: q.id })
        );
    ...
    async updateQuestion() {
      try {
        if (
          this.formUpdateQuestion.type === "radio" ||
          this.formUpdateQuestion.type === "checkbox"
        ) {
          this.formUpdateQuestion.answerLines = this.formUpdateQuestion.answerLines.filter(
            line => line.content != ""
          );
        }
        ...
        let q = await this.$store.dispatch(
          ...
          Object.assign({}, this.formUpdateQuestion)
        );
    ...
    //             let q = Object.assign({}, {content: this.selectedQuestion.content, surveyId: this.form.id, id: this.form.questions[index].id})
    //             this.$store.dispatch('updateSurveyQuestion', Object.assign({}, q))
    //         }
    //     }
    ...
    removeQuestion(index, question) {
      ...
      MessageBox.confirm("Bạn có chắc chắn xóa câu hỏi không?", "Warning", {
        ...
        .then(() => {
          this.$store.dispatch("removeSurveyQuestion", question.id).then(
            res => {
              this.$store.dispatch("fetchSurveyDetail", this.$route.params.id);
              Message({
                message: "Đã xóa",
      ...
      try {
        ...
        if (result) {
          ...
          await this.$store.dispatch(
            "updateSurvey",
            Object.assign({}, this.form, { startDate, expiredDate })
          );
          const id = this.$route.params.id;
