<template>
  <div class="">
    <my-card raised class="form-card">
      ...
      <div class="mb-4">
        ...
        <span
          ...
          :class="{
            'badge-success': review.status === 'approved',
            'badge-danger': review.status === 'rejected',
            'badge-default': review.status === 'pending'
          }"
        >
          {{ review.status }}
        </span>
      ...
      <div class="mb-4">
        ...
        <i
          v-for="(item, index) in Array.from({ length: review.rating })"
          :key="index"
      ...
      <div class="mb-4" v-if="review.images && review.images.length">
        ...
        <ul class="image-list">
          <li v-for="(reviewImage, index) in review.images" :key="index">
            <a
              target="_blank"
              :href="reviewImage.src"
              :style="{ backgroundImage: 'url(' + reviewImage.large + ')' }"
            ></a>
          </li>
        </ul>
      </div>
    ...
    >
      <el-form
        :model="dialogFormReply.form"
        label-width="140px"
        data-vv-scope="dialogFormReply.form"
      >
        <el-form-item :label="$t('el.formCard.content')">
          ...
          <span
            class="text-danger"
            v-if="errors.has('dialogFormReply.form.content')"
            >{{ errors.first("dialogFormReply.form.content") }}</span
          >
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogFormReply.visible = false">{{
          $t("el.button.cancel")
        }}</el-button>
...
export default {
  ...
  computed: {
    replyRows() {
      ...
      const rows = replies.map(reply => {
        return Object.assign({}, reply, {
          userFullName: reply.user && reply.user.fullname,
          reviewStatus: reply.status
        });
      });
    ...
    review() {
      ...
      return Object.assign({}, review, {
        userFullName: review.user && review.user.fullname
      });
    },
    actionsTable() {
      ...
      const hasPendingReply = !!this.$store.state.reviewReplies.find(
        reply => reply.status === "pending"
      );
      return hasPendingReply && this.review.status === "approved"
        ? defaultActionsTable
        : [];
    }
  },
  ...
  methods: {
    ...
    async createReply() {
      try {
        await this.$store.dispatch(`createReviewReply`, {
    ...
    async batchAction(rows, action) {
      try {
        await Promise.all(
          rows.map(r =>
            this.$store.dispatch(`${action}ReviewReply`, {
              reviewId: this.$route.params.id,
              replyId: r.id
            })
          )
