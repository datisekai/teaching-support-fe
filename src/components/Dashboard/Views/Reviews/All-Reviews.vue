...
export default {
  ...
  computed: {
    reviewRows() {
      const rows = this.$store.getters.reviews.map(r => {
        ...
        r.content = r.content.split(' ').slice(0, 50).join(' ');
        const replyStat = r.replies.reduce((stat, reply) => {
          stat[reply.status]++;
          return stat;
  ...
  data() {
    const initFiledArrays = ['id', 'productName', 'content', 'replyStat', 'reviewStatus', 'updatedAt'];
    return {
      filterOutput: [],
  ...
  methods: {
    ...
    async batchAction(rows, action) {
      try {
        await Promise.all(rows.map(r => this.$store.dispatch(`${action}Review`, r.id)));
        await this.$store.dispatch('fetchReviews');
        Notification ({
          title: 'Success',
