<template>
  <div class="row">
    ...
    <el-dialog
      width="500px"
      :title="
        formCreateError.id ? $t('el.button.update') : $t('el.button.create')
      "
    ...
    >
      <el-form :model="formCreateError" data-vv-scope="formCreateError">
        <el-form-item
          v-if="formCreateError.type == 'detail_error'"
          :label="$t('el.formCard.error')"
        >
          <el-select
            ...
            :class="
              errors.has('formCreateError.parentId') ? 'border-danger' : ''
            "
            style="width: 100%"
          >
            <el-option
          ...
          <span
            class="text-danger"
            v-if="errors.has('formCreateError.parentId')"
            >{{ errors.first("formCreateError.parentId") }}</span
          >
        </el-form-item>

        <el-form-item
          v-if="formCreateError.type == 'error'"
          :label="$t('el.formCard.name')"
        >
          <el-input
          ...
          ></el-input>
          <span class="text-danger" v-if="errors.has('formCreateError.name')">{{
            errors.first("formCreateError.name")
          }}</span>
        </el-form-item>
        <el-form-item
          v-if="formCreateError.type == 'detail_error'"
          :label="$t('el.formCard.errorCode')"
        >
          <el-input
            :disabled="!!formCreateError.id"
          ...
          ></el-input>
          <span class="text-danger" v-if="errors.has('formCreateError.code')">{{
            errors.first("formCreateError.code")
          }}</span>
        </el-form-item>
        <el-form-item
          v-if="formCreateError.type == 'detail_error'"
          :label="$t('el.formCard.description')"
        >
          <el-input
            data-vv-name="formCreateError.name"
            v-validate="'required'"
  ...
  methods: {
    ...
    storeProductError() {
      this.$validator.validateAll("formCreateError").then(result => {
        if (result) {
          this.$store.dispatch("storeProductError", this.formCreateError).then(
            res => {
              Notification({
                title: "Success",
    ...
    updateError() {
      this.$validator.validateAll("formCreateError").then(result => {
        if (result) {
          this.$store.dispatch("updateProductError", this.formCreateError).then(
            res => {
              Notification({
                title: "Success",
