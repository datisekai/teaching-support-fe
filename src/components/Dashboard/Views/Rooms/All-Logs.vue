...
import studentSchemas from "./log-schemas";
...
export default {
  ...
  computed: {
    students() {
      const rows = this.$store.state.logStudents.map(c => ({
        ...c.student,
  ...
  mounted() {
    this.$store.dispatch("fetchLogStudents", this.$route.params.id);
    this.$store.dispatch("setPageTitle", "logStudents");
