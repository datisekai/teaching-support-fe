<template lang="html">
  <div class="row">
      ...
      <el-card>
        ...
        <el-table border :data="warrantyItems" class="eye-table">
          ...
          <el-table-column
            ...
            <template slot-scope="props">
              <el-checkbox :disabled="formQuotation.status !== 'new'" v-model="props.row.stamp" @change="updateWarrantyItem(props.row)"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column
            :label="$t('el.tableHeader.dkbh')">
            <template slot-scope="props">
              <el-checkbox :disabled="formQuotation.status !== 'new'" v-model="props.row.isDKBH" @change="updateWarrantyItem(props.row)"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column
            prop="times"
        ...
        <el-form :model="formReceipt" label-position="top">
          <el-form-item :label="$t('el.formCard.accessories')">
            <el-input v-model="formReceipt.accessories" :disabled="formReceipt.status == 'completed'"></el-input>
          </el-form-item>
          <div class="row">
            <div class="col-6">
    ...
    <el-dialog 
      width="400px" 
      :title="formAddError.id ? $t('el.dialogTitle.editResultError') : $t('el.dialogTitle.addError')" 
      :visible.sync="modal.addError"  
      @keyup.enter.native="addError"
    >
      <el-form :model="formAddError" data-vv-scope="formAddError">
        <el-form-item>
          <el-select style="width: 100%" v-model="formAddError.warrantyItemId" placeholder="Chọn mã/tên máy" disabled>
            <el-option v-for="item in warrantyItems" 
              :value="item.id" 
              :label="item.warrantyCode && item.warrantyCode.code || item.name">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
          <select-ajax
            ...
            :attribute="{type: 'product_error', field: 'name', key: 'id', valueKey: 'id', multiple: false, initOptions: detailErrors('error'), extraQuery: { parentId: parentError && parentError.id }}"
          >
          </select-ajax>
        </el-form-item>
        <el-form-item>
          <select-ajax
            ...
            :data-vv-as="$t('el.formCard.errorDescription')"
            :class="errors.has('formAddError.warrantyTypeId')?'border-danger':''"
            :attribute="{type: 'product_error', field: 'name', key: 'id', valueKey: 'id', multiple: false, initOptions: detailErrors('suggestion_error'), extraQuery: { parentId: parentSuggestionError && parentSuggestionError.id }}"
          >
          </select-ajax>
        </el-form-item>
        <el-form-item>
          <select-ajax
            ...
            :data-vv-as="$t('el.formCard.errorDescription')"
            :class="errors.has('formAddError.warrantyTypeId')?'border-danger':''"
            :attribute="{type: 'product_error', field: 'name', key: 'id', valueKey: 'id', multiple: false, initOptions: detailErrors('result_error'), extraQuery: { parentId: parentResultError && parentResultError.id }}"
          >
          </select-ajax>
        </el-form-item>
        <el-form-item>
          <select-ajax
            ...
            :data-vv-as="$t('el.formCard.errorDescription')"
            :class="errors.has('formAddError.warrantyTypeId')?'border-danger':''"
            :attribute="{type: 'product_accessory', field: 'name', key: 'id', valueKey: 'id', multiple: false, initOptions: defaultProductAccessories, extraQuery: {productLineId: formAddError.productLineId}}"
          >
          </select-ajax>
        </el-form-item>
        <el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="modal.addError = false">{{$t('el.button.cancel')}}</el-button>
        <el-button v-if="!formAddError.id" type="primary" @click="addWarrantyLine" >{{$t('el.button.add')}}</el-button>
        <el-button v-else type="primary" @click="editWarrantyLine" >{{$t('el.button.update')}}</el-button>
      </span>
    </el-dialog>
    <el-dialog
      width="400px"
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="modal.complete = false">{{$t("el.button.cancel")}}</el-button>
        <el-button
          type="danger"
...
<script>
import {Notification, MessageBox, Input, Button, Form, FormItem, Select, Option, Card, Table, TableColumn, Checkbox, RadioGroup, Radio, Dialog } from 'element-ui';
...
export default {
  ...
  data() {
    ...
    const columnDefs = dtHelper.buildInitFields(quotationFoldableForm, initFiledArrays);
    const tableActions = [];
  ...
  computed: {
    foldableTableData() {
      return this.warrantyItems.map((item, index) => {
        ...
        item.children = item.warrantyLines.map(line => {
          ...
          const error = this.detailErrors('error').find(i => i.id === line.productErrorId);
          const resultError = this.detailErrors('result_error').find(i => i.id === line.productResultErrorId);
          const suggestionError = this.detailErrors('suggestion_error').find(i => i.id === line.productSuggestionErrorId);
    ...
    warrantyLines(){
      return this.warrantyItems.reduce((cur, i) => cur.concat(i.warrantyLines), []);
    },
    parentError() {
      return this.$store.state.productErrors.find(i => i.type === 'error');
    },
    parentSuggestionError() {
      return this.$store.state.productErrors.find(i => i.type === 'suggestion_error');
    },
    parentResultError() {
      return this.$store.state.productErrors.find(i => i.type === 'result_error');
    },
    models() {
  ...
  methods: {
    ...
    showModalEditError(index, row) {
      this.defaultProductAccessories = this.detailAccessories.filter(i => i.productLineId === row.productLineId);
    ...
    async updateCustomerAgree(agreeToFix) {
      ...
      if (this.formQuotation.status == 'new') {
        try {
          await this.$store.dispatch('updateQuotation', { id: this.formQuotation.id, agreeToFix: this.formQuotation.agreeToFix });
        }
        catch(e) {
          Notification ({
            title: 'Error',
    ...
    async addWarrantyLine() {
      try {
        let res = await this.$store.dispatch('addWarrantyLine', {id: this.formMaintenance.id, data: this.formAddError});
        Notification ({
          title: 'Success',
        ...
        const warrantyItem = this.warrantyItems.find(i => i.id === res.warrantyItemId);
        if (warrantyItem) {
    ...
    async editWarrantyLine() {
      try {
        let res = await this.$store.dispatch('updateWarrantyLine', this.formAddError);
        Notification ({
          title: 'Success',
        ...
        const warrantyItem = this.warrantyItems.find(i => i.id === res.warrantyItemId);
        if (warrantyItem) {
          const index = warrantyItem.warrantyLines.findIndex(l => l.id === res.id);
          warrantyItem.warrantyLines[index] = Object.assign(warrantyItem.warrantyLines[index], res);
        }
        this.modal.addError = false;
      }
      catch(e) {
    ...
    async createInvoice() {
      MessageBox.confirm('Bạn có chắc chắn muốn tạo phiếu thu?', 'Warning', {
        ...
        try {
          let res = await this.$store.dispatch('storeInvoice', { quotationFormId: this.formQuotation.id})
          Notification ({
            title: 'Success',
    ...
    async createReimbursement() {
      MessageBox.confirm('Bạn có chắc chắn muốn hoàn trả?', 'Warning', {
        ...
        try {
          let res = await this.$store.dispatch('storeReimbursement', { quotationFormId: this.formQuotation.id})
          Notification ({
            title: 'Success',
   ...
   watch: {
    ...
    'formQuotation.status': function(newVal) {
      ...
      if (newVal == 'completed') {
        const total = this.warrantyLines.reduce((cur, l) => cur + l.price, 0) || 0;
        if (total != 0) {
          currentActions.push({
            label: this.$t('el.button.invoice'),
            type: 'warning',
            icon: '',
            disabled: this.formQuotation.invoiceForm || this.$util.checkPermissionAction(permissionSchemas, 'create_invoice') ? false : true,
            callback: () => {
              if (this.formQuotation.invoiceForm) {
                this.$router.push('/invoice/'+this.formQuotation.invoiceForm.id)
              }
              else {
                this.createInvoice();
              }
        ...
        if (this.$util.checkPermissionAction(permissionSchemas, 'create_reimbursement') && total === 0) {
          currentActions.push({
