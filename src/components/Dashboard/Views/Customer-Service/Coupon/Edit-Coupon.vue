        ...
        <div class="card-body form-card">
          ...
          <div class="row">
            <div class="col-sm-12 p-0">
              ...
              <span v-if="csCouponDetail.isExpired" class="p-badge badge-success badge-pill ml-2">
                ✔
              </span>
      ...
      <div class="card">
        ...
        <div class="card-body form-card">
          ...
          <div v-if="form.discountType != 'free_shipping'"> 
            ...
            <div class="row">
              <div class="col-sm-12 p-0">
                <label :class="csCouponDetail.type !== 'birthday' ? 'required' : ''">{{$t('el.formCard.applyTo')}}</label>
                <my-select
                  v-if="csCouponDetail.type !== 'birthday'"
                  :disabled="!canEdit"
                  class="full-width"
...
export default {
  ...
  computed: {
    ...mapState(['csCouponDetail', 'noImage']),
    provinces() {
    ...
    canEdit() {
      return !this.csCouponDetail.isExpired;
    },
    currentTypeRelationTitle() {
      const typeRelation = this.$util.getOptions('typeRelation').find(t => t.value === this.csCouponDetail.typeRelation);
  ...
  methods: {
    async init() {
      ...
      await this.$store.dispatch('fetchCSCouponDetail', id);
      console.log('---coupon detail', this.csCouponDetail);
    },
    updateSTT() {
    ...
    save() {
      this.$validator.validateAll().then((result) => {
        if (result) {
          ...
          this.$store.dispatch('updateCSCoupon', data).then((res) => {
            Notification ({
              title: 'Success',
  ...
  watch: {
    ...
    'form.typeRelation': function(newVal) {
      if (newVal === 'category') {
        this.form.productIds = this.csCouponDetail.productIds;
      } else if (newVal === 'product') {
        this.form.categoryIds = this.csCouponDetail.categoryIds;
      }
    },
    async csCouponDetail(newVal) {
      if (newVal) {
        this.form = {
          ...this.form,
          ...newVal,
