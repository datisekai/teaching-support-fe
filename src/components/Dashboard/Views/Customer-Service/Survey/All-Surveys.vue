...
export default {
  ...
  mounted() {
    this.$store.dispatch("fetchSurveys", { department: "customer_service" });
  ...
  methods: {
    create() {
      this.$router.push("/cs/survey/create");
    },
    edit(index, row) {
      this.$router.push(`/cs/survey/${row.id}`);
    },
    remove(index, row) {
      MessageBox.confirm("Bạn có chắc chắn xóa bài viết không?", "Warning", {
        ...
        .then(() => {
          this.$store.dispatch("removeSurvey", row.id).then(
            res => {
              Message({
                message: "Đã xóa",
                type: "success"
    ...
    updateStatus(rows, status) {
      const self = this;
      this.$util
        .updateStatusAll("survey", rows, status)
        .then(result => {
