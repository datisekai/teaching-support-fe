<template>
  <div class="row">
    ...
    <el-dialog 
      width="500px" 
      :title="formCreateModel.id ? $t('el.button.update') : $t('el.button.create')" 
      :visible.sync="dialog.formCreateVisible" 
      @closed="handleClosed"
      @close="handleCloseCreateModel"
      @keyup.enter.native="storeModel"
    >
      <el-select v-if="!formCreateModel.id" v-model="formCreateModel.type" style="width: 100%">
        <el-option value="model" :label="$t('el.formCard.model')"></el-option>
        <el-option value="product_line" :label="$t('el.formCard.productLine')"></el-option>
      </el-select>
      <el-form :model="formCreateModel" data-vv-scope="formCreateModel">
        <el-form-item v-if="formCreateModel.type == 'product_line'" :label="$t('el.formCard.model')">
          <el-select 
            ...
            :class="errors.has('formCreateModel.parentId')?'border-danger':''"
            style="width: 100%">
            <el-option v-for="item in modelMenus" :key="item.value" :value="item.value" :label="item.label"></el-option>
          </el-select>
          <span class="text-danger" v-if="errors.has('formCreateModel.parentId')">{{ errors.first('formCreateModel.parentId') }}</span>
        </el-form-item>
        
         <el-form-item :label="$t('el.formCard.sku')" v-if="formCreateModel.type == 'product_line'">
          <el-input
            disabled
            v-model="formCreateModel.model"
          ></el-input>
        </el-form-item>
        <el-form-item :label="$t('el.formCard.name')">
          ...
          ></el-input>
          <span class="text-danger" v-if="errors.has('formCreateModel.name')">{{ errors.first('formCreateModel.name') }}</span>
        </el-form-item>
       <el-form-item label="SKU" >
          ...
          ></el-input>
          <span class="text-danger" v-if="errors.has('formCreateModel.sku')">{{ errors.first('formCreateModel.sku') }}</span>
        </el-form-item>
        <el-form-item :label="$t('el.formCard.expiredTime')">
           <el-input
            ...
            :class="errors.has('formCreateModel.expiredIn')?'border-danger':''"
          ></el-input>
          <span class="text-danger" v-if="errors.has('formCreateModel.expiredIn')">{{ errors.first('formCreateModel.expiredIn') }}</span>
        </el-form-item>
        <el-form-item   v-if="formCreateModel.type == 'product_line'"  :label="$t('el.formCard.note')">
           <el-input
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" v-if="formCreateModel.id" @click="updateModel" :disabled="disabledForm">{{$t('el.button.update')}}</el-button>
        <el-button type="primary" v-else @click="storeModel" :disabled="disabledForm">{{$t('el.button.create')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    const initFiledArrays = ['id', 'modelName', 'productLine', 'sku', 'expiredTime'];
  ...
  methods: {
    ...
    storeModel() {
      this.$validator.validateAll('formCreateModel').then((result) => {
        if (result) {
          this.formCreateModel.expiredIn = parseInt(this.formCreateModel.expiredIn);
          this.$store.dispatch('storeModel', this.formCreateModel).then((res) => {
            Notification ({
              title: 'Success',
    ...
    updateModel() {
      this.$validator.validateAll('formCreateModel').then((result) => {
        if (result) {
          MessageBox.confirm('Thời gian kết thúc của các mã bảo hành liên quan có thể bị thay đổi, bạn có chắc chắn cập nhật không?', 'Warning', {
            ...
            this.formCreateModel.expiredIn = parseInt(this.formCreateModel.expiredIn);
            this.$store.dispatch('updateModel', this.formCreateModel).then((res) => {
              Notification ({
                title: 'Success',
