<template>
  <div class="row">
    ...
    <el-dialog
      ...
      :visible.sync="dialogAddBlogVisible"
      width="40%"
      @keyup.enter.native="addBlogAll"
    >
      <el-checkbox-group v-model="sellectAddBlogs">
        <div v-for="(blog, index) of blogs" :key="index">
          <el-checkbox :label="blog.id" name="type">{{
            blog.title
          }}</el-checkbox>
        </div>
      </el-checkbox-group>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addBlogAll"
          >Thêm vào danh mục</el-button
        >
      </span>
    </el-dialog>
    <el-dialog
      title="Xóa khỏi danh mục"
      :visible.sync="dialogRemoveBlogVisible"
      width="40%"
      @keyup.enter.native="removeBlogAll"
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="removeBlogAll"
          >Xóa khỏi danh mục</el-button
        >
      </span>
    </el-dialog>
  </div>
    ...
    addBlogAll() {
      ...
      if (blog_ids && blog_ids.length) {
        Promise.all(
          blog_ids
            .map(blog_id =>
              this.currentSelectRow.map(article => {
