<template>
  <div class="row">
    ...
    <el-dialog
      width="500px"
      :title="
        formCreateAccessory.id ? $t('el.button.update') : $t('el.button.create')
      "
      ...
      <el-form :model="formCreateAccessory" data-vv-scope="formCreateAccessory">
        <el-form-item :label="$t('el.formCard.product')">
          <my-select
            ...
            :class="
              errors.has('formCreateAccessory.productLineId')
                ? 'border-danger'
                : ''
            "
          >
          ...
          <span
            class="text-danger"
            v-if="errors.has('formCreateAccessory.productLineId')"
            >{{ errors.first("formCreateAccessory.productLineId") }}</span
          >
        </el-form-item>
        <el-form-item :label="$t('el.formCard.productAccessories')">
          <el-input
            ...
            :class="
              errors.has('formCreateAccessory.name') ? 'border-danger' : ''
            "
          ></el-input>
          <span
            class="text-danger"
            v-if="errors.has('formCreateAccessory.name')"
            >{{ errors.first("formCreateAccessory.name") }}</span
          >
        </el-form-item>
        <el-form-item :label="$t('el.formCard.price')">
          <my-money
            ...
            :class="
              errors.has('formCreateAccessory.price') ? 'border-danger' : ''
            "
          ...
          <span
            class="text-danger"
            v-if="errors.has('formCreateAccessory.price')"
            >{{ errors.first("formCreateAccessory.price") }}</span
          >
        </el-form-item>
      </el-form>
...
export default {
  ...
  computed: {
    ...
    disabledForm() {
      return (
        !this.formCreateAccessory.productLineId ||
  ...
  methods: {
    ...
    storeProductAccessory() {
      this.$validator.validateAll("formCreateAccessory").then(result => {
        if (result) {
          this.$store
            .dispatch("storeProductAccessories", this.formCreateAccessory)
            .then(
              res => {
                Notification({
                  title: "Success",
    ...
    updateAccessory() {
      this.$validator.validateAll("formCreateAccessory").then(result => {
        if (result) {
          this.$store
            .dispatch("updateProductAccessories", this.formCreateAccessory)
            .then(
              res => {
                Notification({
                  title: "Success",
